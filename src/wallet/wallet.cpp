...
// donu: optional setting to unlock wallet for block minting only;
...
//    return n ? n : nTimeReceived;
    ...
    // donu: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// donu: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // donu: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // donu: set time
...
//                }
                ...
                // donu: sub-cent change is moved to fee
...
//donuTODO: this code was in 0.7, but was refactored by b33d1f5ee512da5719b793b3867f75f1eea5cf52:
//                int64 nPayFee;
//                if (fNewFees)
...
//                    nPayFee = nTransactionFee * (1 + (int64)nBytes / 1000);
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount minimum_value_for_change = MIN_TXOUT_AMOUNT;  //donuTODO - is this correct?
...
// donu: create coin stake transaction
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew)
{
    ...
    for (const auto& pcoin : setCoins)
    {
        ...
        for (unsigned int n=0; n<std::min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, chainActive.Tip(), header, postx.nTxOffset + CBlockHeader::NORMAL_SERIALIZE_SIZE, tx, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
                ...
                if (nCredit >= nSplitThreshold && header.GetBlockTime() + nStakeSplitAge > txNew.nTime)
                    txNew.vout.push_back(CTxOut(0, scriptPubKeyOut)); // @dev: creates empty vout for splitting stake #nSplitThreshold
                if (gArgs.GetBoolArg("-debug", false) && gArgs.GetBoolArg("-printcoinstake", false))
    ...
    for (const auto& pcoin : setCoins)
    {
            ...
            && pcoin.outpoint.hash != txNew.vin[0].prevout.hash)
        {
            ...
            if (txNew.vin.size() >= 100)
                break;
            // Stop adding more inputs if value is greater than/equal to nCombineThreshold
            if (nCredit >= nCombineThreshold)
                break;
            // Stop adding more inputs if reserve limit is reached
            if (nCredit + pcoin.txout.nValue > nBalance - nReserveBalance)
                break;
            // Don't add additional input if greater than/equal to nCombineThreshold
            if (pcoin.txout.nValue >= nCombineThreshold)
                continue;
            // Don't add additional input if nStakeMinAge requirements are not met
    ...
    {
        // Set output amount
        // @dev: Fill nCredit vout(s) according to #nSplitThreshold
