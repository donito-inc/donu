...
// donuoin: sync-checkpoint master key
...
// donu: synchronized checkpoint (centrally broadcasted)
...
// donu: get last synchronized checkpoint
...
// donu: only descendant of current sync-checkpoint is allowed
...
bool CheckSyncCheckpoint(const uint256& hashBlock, const CBlockIndex* pindexPrev)
{
   ...
   if (pindexPrev->nHeight > 0 && chainActive.Height() == 0) return true;
    ...
    //donuTODO - needs rewrite, because it is very slow during accepting headers
    return true;
...
// donu: reset synchronized checkpoint to last hardened checkpoint
...
// donu: verify signature of sync-checkpoint message
...
// donu: process synchronized checkpoint
