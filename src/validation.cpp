...
const std::string strMessageMagic = "Donu Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // donu: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
        // donuTODO move back to policy.h after 0.8 is active
...
int64_t GetProofOfWorkReward(int nBlockHeight)
{
    int64_t nSubsidy;
    ...
    if (nBlockHeight == 3) {
        nSubsidy = 6834694.727139 * COIN; // Swap
    } else if (nBlockHeight == 5) {
        nSubsidy = 504000 * COIN;           // Internal staking
    } else if (nBlockHeight > 5 && nBlockHeight <= 43200) {
        nSubsidy = 2 * COIN;                // ~60 days of public mining
    } else {
        nSubsidy = 0 * COIN;
    }
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // donu: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // donu
            undo.nTime = alternate.nTime;           // donu
        } else {
...
bool DonuContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    uint256 hashProofOfStake = uint256();
    // donu: verify hash target and signature of coinstake tx
    ...
    // donu: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // donu: compute stake modifier
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    */
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !DonuContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // donu: coinbase reward check relocated to CheckBlock()
    ...
    // donu: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // donu: fees are not collected by miners as in bitcoin
    // donu: fees are destroyed to compensate the entire network
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // donu: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // donu: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // donu: check transaction timestamp
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
...
#ifdef ENABLE_CHECKPOINTS
    // donu: check that the block satisfies synchronized checkpoint
    ...
    // Strictly enforce block version 3
    if(block.nVersion != 3 && IsProtocolV06(pindexPrev))
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // donu: Don't reject in case of old clients. Change our assumption instead.
        //donuTODO: Maybe add restrictions until when this is allowed? We don't want new clients to pretend to be old clients and try to abuse this.
    ...
    //donuTODO - move this somewhere in the upper calls, where pfrom is visible
//    // donu: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // donu: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // donu: check PoS
    if (fCheckPoS && !DonuContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // donu: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // donu: if responsible for sync-checkpoint send it
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // donu: calculate stake modifier checksum
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // donu: txindex is always enabled
...
// donu: total coin age spent in transaction, in the unit of coin-days.
