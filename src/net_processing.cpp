...
namespace {
    ...
    /** donu: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (inv.hash == pfrom->hashContinue)
        {
            ...
            // donu: send latest proof-of-work block to allow the
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    if (!ProcessNewBlockHeaders(pfrom->lastAcceptedHeader, headers, pfrom->nVersion <= OLD_VERSION, state, chainparams, &pindexLast, &first_invalid_header)) {
        int nDoS;
        if (state.IsInvalid(nDoS)) {
            LOCK(cs_main);
            if (nDoS > 0) {
...
{
    if (chainActive.Height() + 1 >= chainparams.GetConsensus().nDGWForkHeight && pfrom->nVersion > INIT_PROTO_VERSION && pfrom->nVersion <= PREFORK_PROTO_VERSION) {
    ...
    // donu: set/unset network serialization mode for new clients
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // donu: tell downloading node about the latest block if it's
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!ProcessNewBlockHeaders(chainActive.Tip()->GetBlockHash(), {cmpctblock.header}, false, state, chainparams, &pindex)) {
            int nDoS;
            if (state.IsInvalid(nDoS)) {
                if (nDoS > 0) {
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
...

        {
            ...
            // donu: store in memory until we can connect it to some chain
        ...
        // donu: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // donu: try to select next block in a constant time
