...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        // donu: enforce minimum output amount
...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const Consensus::Params& params)
{
    ...
    if (tx.IsCoinStake())
    {
        // donu: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
        ...
        if (nStakeReward > GetProofOfStakeReward() - nCoinstakeCost)
    ...
    else
    {
        ...
        // donu: enforce transaction fees for every block
