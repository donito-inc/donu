...
UniValue GetNetworkHashPS(int lookup, int height) {
    ...
    //donuTODO - redo this to fit donu
...
// donu: get network Gh/s estimate
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when donud is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    // donu: check block before attempting to sign it
    CValidationState state;
